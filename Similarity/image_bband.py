...
def image_bband(save_root, bollinger_img_path, sorted_keys, start_idx):
    """볼린저 밴드 이미지 IOU 계산 함수 (멀티프로세싱용, start_idx부터 끝까지 처리)"""
    os.makedirs(save_root, exist_ok=True)
    
    # 모든 이미지를 한 번만 읽어서 메모리에 저장 (start_idx 이후만 계산하지만, 비교는 0부터)
    image_cache = {key: read_image(os.path.join(bollinger_img_path, key)) for key in sorted_keys}
    
    # start_idx부터 마지막 인덱스까지 처리
    for i in tqdm(range(start_idx, len(sorted_keys)), desc=f'Calculating Bollinger Band IOU (from {start_idx} to end)'):
        try:
...
def calculate_miou(target_image, comparison_image):
    ...
    miou = np.sum(intersection) / np.sum(union) if np.sum(union) > 0 else 0  # 분모가 0인 경우 처리
    return miou