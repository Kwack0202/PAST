...
def plot_long_short_signals(file, base_dir='./Backtesting/simulation/', output_base='./Backtesting/plots/'):
    ...
    long_signal = df[df['position'] == 'long']
    short_signal = df[df['position'] == 'short']
    margin_signal = df[df['position'] == 'margin transaction']
    ...
    plt.title(f'Long/Short Signals - {file}')
    plt.xlabel('Index')
    plt.ylabel('Price')
    plt.legend()
    plt.grid(True)
    ...
    plt.savefig(f'{output_dir}/full_period.png')
    plt.close()
    ...
    for month in range(1, 13):
        ...
        plt.savefig(f'{output_dir}/{month}_signal.png')
        plt.close()
...
def plot_margin_transaction_return(file, base_dir='./Backtesting/simulation/', output_base='./Backtesting/plots/'):
    ...
    # 전체 기간 시각화
    plt.figure(figsize=(20, 12))
    margin_data = df[df['position'] == 'margin transaction']
    ...
    plt.title(f'Margin Transaction Return - {file}')
    plt.xlabel('Index')
    ...
    for month in range(1, 13):
        ...
        plt.savefig(f'{output_dir}/{month}_return.png')
        plt.close()
